...
export class Envelope implements Envelope.Properties {
	...
	get properties(): Envelope.Properties {
		const { version, keyId, dhPublicKey, messageNumber, previousChainLength, kemCiphertext, cipherData, rSignature } =
			this;
	...
	validate(): void {
		...
		if (this.kemCiphertext && this.kemCiphertext.byteLength !== MlKemCipherTextCodec.byteLength()) {
			throw new Error(
				`Invalid kemCiphertext length: ${this.kemCiphertext.byteLength}, expected ${MlKemCipherTextCodec.byteLength()}`,
			);
		}
	}
